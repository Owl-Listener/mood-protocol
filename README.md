...
## Generating images from mood.md

`mood.md` contains everything you need to write a strong image generation prompt — colour with hex values, texture, emotional register, references, and anti-references all map directly to prompt elements.

→ [prompting.md](./prompting.md) — How to translate a mood.md into image generation prompts. Covers prompt structure, section mapping, and tool-specific syntax for Midjourney, DALL-E 3, Firefly, and Stable Diffusion.
...
- **Prompting guides** — tool-specific extensions to [prompting.md](./prompting.md) for new image generation tools

## See also

→ [voice-protocol](https://github.com/Owl-Listener/voice-protocol) — The verbal equivalent: how your brand writes, for any AI agent.  
→ [tokens-protocol](https://github.com/Owl-Listener/tokens-protocol) — The systematic layer: design token intent so agents make the right choices when generating UI.
